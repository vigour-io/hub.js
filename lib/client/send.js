...
const parse = (stamp, hub, t) => {
  ...
  if (!src) {
    ...
    // if (hub.clients[src]) {
      // need another list of 'some clients have clients deeper'
      // console.log('!!! ok !!!', src) // need to support more levels ofc
    // }
    if (!hub.client.src) {
      hub.client.src = {}
    }
    ...
    t[1].src[src] = hub.client.src[src] = true
    return stamp
  }
}
    ...
    if (hub && !hub.receiveOnly && struct.key !== 'clients') {
      ...
      if (struct === hub) {
        ...
        if (struct.key !== hub.id) {
          console.log(hub.client.src, struct.key)
          if (hub.client.src && hub.client.src[struct.key]) {
            const store = inProgress(hub, bs.on)
            if (!store[1]) store[1] = {}
            if (!store[1].src) store[1].src = {}
            store[1].src[struct.key] = null
          }
          // keep track of waht you send
          return
        }
      }
