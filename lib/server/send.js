...
const serialize = (id, client, t, subs, struct, val, level) => {
  ...
  console.log('GO GO GO', subs, struct.path())
  ...
  if ((val === null || !(cached = isCached(client, struct, stamp))) || subs.val === true) {
    ...
    if (src !== client.key && bs.src(t.tStamp) !== client.key) {
      if (
        ...
        // its cached since were sending it from a previous subs need to take care...

        // wrong behvaiour just allwats go deep when val: true why not?
        ...
        if (!cached && (struct.val !== void 0 || val === null)) {
          ...
          if (val === null) {
            ...
            if (struct.key === 'type') {
              handleType(struct, id, client, t, subs, val, level)
            }
            // will send too much now -- have to exclude everything in type -- bit harder
...
const handleType = (struct, id, client, t, subs, val, level) => {
  const type = getType(struct.parent().parent(), struct.compute())
  serialize(id, client, t, subs, type, val, level)
}
