...
const send = (hub, client, t, type, subs, tree) => {
  ...
  if (type === 'remove') {
    ...
    if (tree.$t && tree.$t._p && !tree.$t._p[tree.$t.key]) {
      serialize(hub.id, client, client.inProgress, subs, tree.$t, null, hub.serverIndex, tree)
    }
  }
  if (t.val !== void 0 || val === null || subs.val === true) {
    serialize(hub.id, client, client.inProgress, subs, t, val, hub.serverIndex, tree)
  }
}
...
const serialize = (id, client, t, subs, struct, val, level) => {
  ...
  if (stamp) {
    ...
    if (client.cache) {
      if (val === null) {
        if (client.cache[uid]) {
          // console.log('REMOVE FROM CACHE', val, struct.path())
          delete client.cache[uid]
        }
      } else if (client.cache[uid] === stamp) {
        // console.log('BLOCKED BY CACHE', client.key, id, client.cache, val, struct.path())
        return true
      }
    }
    ...
    if (src !== client.key && bs.src(t.tStamp) !== client.key) {
      if (client.resolve && client.resolve[src] && Number(bs.val(stamp) >= client.resolve[src])) {
        ...
        // console.log('BLOCKED BY RESOLVE', client.key, id, Number(bs.val(stamp)), client.resolve, client.cache)
      } else {
