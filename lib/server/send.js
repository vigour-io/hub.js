...
const serialize = (id, client, t, subs, struct, val, level) => {
  ...
  if (stamp) {
    ...
    if (client.cache && client.cache[uid] === stamp) {
      if (val === null) delete client.cache[uid]
      console.log('BLOCKED BY CACHE', client.key, id, client.cache)
    } else {
