...
const serialize = (id, client, t, subs, struct, val, level) => {
  ...
  if (stamp && (val === null || !(cached = isCached(client, struct, stamp))) || subs.val === true) {
    // console.log('X -> GO SEND', struct.path(), 'TO', client.device ? client.device.compute() : 'SOMETHING WEIRD', client.key, client.val)
    // if (!client.device) {
      ...
      if (
        ...
        if (isType) { // means its blocked otherwise (could be a set form own client)
          ...
          for (let i = level; i < len; i++) {
            ...
            if (!tt) {
              if (!s[path[i]]) {
                console.log('CANT BE STOPPED', path, path[i], s)
              }
              s = s[path[i]] = {}
            } else {
              s = tt
