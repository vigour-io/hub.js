  ...
  data: {
    remove$: (val, stamp, struct) => {
      if (val === null) {
        ...
        if (hub) {
          // maybe the whle loop makes it heavier...
          // console.log('---> remove!', x.id)
          if (struct._p) {
            if (!struct._p._removed) {
              struct._p._removed = []
              // really heavy... make a thing remove in progress and add an array
              ...
              removedInProgress.push(struct._p)
            }
