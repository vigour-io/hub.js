...
export default (hub, socket, data) => {
  ...
  if (meta) {
    ...
    if (client) {
      t = hub
      console.log('\n💉 💉 💉 ', meta.context, client.context)
      if ('context' in meta && client.context != meta.context) { // eslint-disable-line
        removeClient(client)
        console.log('🔫 🔫 🔫 !!! 🔫 🔫 🔫', client.parent(2).subscriptions.length, hub.contextKey, client.parent(2).contextKey)
      ...
      console.log('💣 💣 💣 NEW CLIENT 💣 💣 💣', meta.id, socket.useragent)
      t = create(hub, socket, meta)
      client = socket.client
    }
  } else {
